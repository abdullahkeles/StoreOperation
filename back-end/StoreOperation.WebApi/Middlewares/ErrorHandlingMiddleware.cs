...
namespace StoreOperation.WebApi.Middlewares
{
    /// <summary>
    ...
    public class ErrorHandlingMiddleware
    {
        ...
        public async Task Invoke(HttpContext context, ILoggerRepository loggerRepository)
        {
            ...
            {
                await loggerRepository.WriteErrorAsync(error);
                var response = context.Response;
                response.ContentType = "application/json";
                response.StatusCode = (int) HttpStatusCode.InternalServerError;
                

                var resp = new ApiResponse(ResponseStatus.BadRequest, ResultMessage.InternalServerError);
